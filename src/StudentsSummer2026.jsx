...
function StudentCardsSummer26() {
    function classCount(){ 
    
        const count=data.filter((el)=>{return el.cohort.cohortCode==="Summer2026"})
    ...
    const studentsToRender = data.map((eachStudent) => {
        ...
        return (
            ...
            <div className="layout" id="sum26" key={Math.random()*100000}>
...
<div id="table" ><table id ={eachStudent.id}>
        ...
        <tr><td>Goal:{eachStudent.codewars.goal.total}</td> 
            ...
            <td>GitHub:{eachStudent.certifications.github ?hasIt:doesntHaveIt}</td></tr>
                ...
                <section><p className="student-name"><h1>{eachStudent.names.preferredName}  {eachStudent.names.middleName} {eachStudent.names.surname}</h1> </p>
                    ...
                    <p class="track5">{eachStudent.certifications.resume&&eachStudent.certifications.linkedin
    ...
    return (<div className="summer2026" hidden="hidden" ><h1>Summer 2026 Students</h1> <p><strong>Count of Students In Cohort: {classCount()}</strong></p>{studentsToRender}</div>);

}


export default StudentCardsSummer26